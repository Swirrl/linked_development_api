...
describe ThemeService do
  ...
  context "#get_all" do
    describe 'raises error' do
      it 'InvalidDocumentType when type is not one of eldis, r4d, or all' do
        expect {service.get_all({type: 'foo', detail: 'short'}, {:host => 'test.host', :format => :json})}.to raise_error InvalidDocumentType
      end
    end

    context 'eldis' do
      include_examples 'example documents', [:eldis, :get_all, :theme, {:limit => 10, :offset => 0, :host => 'test.host', :format => :json}, {:filename => 'eldis_get_all_theme'}]
    end

    context 'r4d' do
      include_examples 'example documents', [:r4d, :get_all, :theme, {:limit => 10, :offset => 0, :host => 'test.host', :format => :json}, {:filename => 'r4d_get_all_theme'}]
    end
